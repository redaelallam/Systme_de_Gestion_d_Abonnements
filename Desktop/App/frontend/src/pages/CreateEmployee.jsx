...
import {
  ArrowLeft, Save, User, Mail, Lock, Shield, ChevronDown, Info, Loader2,
} from "lucide-react";
...
export default function CreateEmployee() {
  ...
  const [formData, setFormData] = useState({ nom: "", email: "", password: "", role: "employee" });
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      if (createEmployee.fulfilled.match(result)) {
        toast.success(t("clients.clientCreated"), { id: loadingToast });
        ...
        if (payload?.errors) {
          ...
          toast.error(payload?.message || t("common.error"));
        }
      }
      setLoading(false);
    },
    ...
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center animate-in fade-in duration-500">
      ...
      <div className="w-full max-w-xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-foreground tracking-tight">{t("employees.createTitle")}</h1>
            <p className="text-muted-foreground mt-1 text-sm">{t("employees.createSubtitle")}</p>
          </div>
        ...
        <div className="bg-card text-card-foreground rounded-xl shadow-sm border border-border overflow-hidden">
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-5">
              {[
                { name: "nom", label: t("employees.name"), type: "text", icon: User, placeholder: t("employees.namePlaceholder") },
                { name: "email", label: t("employees.email"), type: "email", icon: Mail, placeholder: t("employees.emailPlaceholder") },
              ].map((f) => (
                <div key={f.name}>
                  ...
                  <div className="relative group">
                    ...
                    <input type={f.type} name={f.name} value={formData[f.name]} onChange={handleChange}
                      className={`${inputClass} ${errors[f.name] ? "border-destructive" : ""}`} placeholder={f.placeholder} />
                  </div>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">{t("employees.rolePermissions")}</label>
                <div className="relative group">
                  <div className={iconClass}><Shield size={18} /></div>
                  <select name="role" value={formData.role} onChange={handleChange} className={`${inputClass} appearance-none cursor-pointer pr-10`}>
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input type="password" name="password" value={formData.password} onChange={handleChange}
                    className={`${inputClass} ${errors.password ? "border-destructive" : ""}`} placeholder={t("employees.passwordPlaceholder")} />
                </div>
              ...
              <div className="bg-primary/5 border border-primary/10 p-4 rounded-md flex gap-3 items-start">
                ...
                <p className="text-sm text-foreground/80 leading-relaxed" dangerouslySetInnerHTML={{ __html: t("employees.roleInfo") }} />
              </div>
              ...
              <div className="pt-2 flex justify-end">
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} {t("employees.saveEmployee")}
                </button>
              </div>
            </form>
          </div>
