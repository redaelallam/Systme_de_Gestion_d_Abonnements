      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-border">
            {safeData.length === 0 ? (
              <tr>
                <td colSpan={columns?.length ?? 1} className="px-6 py-8 text-center text-sm text-muted-foreground">
                  {emptyMessage ?? "No data available"}
                </td>
              </tr>
            ) : (
              safeData.map((row, idx) => (
                <tr key={row?.id ?? idx} className="transition-colors hover:bg-muted/30">
                  {columns?.map((col) => (
                    <td key={col.key} className="whitespace-nowrap px-6 py-4 text-sm text-card-foreground">
...
const linkCell = (link) =>
  link ? (
    <a href={link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-info hover:underline">
...
export const EmployeesTable = ({ data }) => {
  const columns = useMemo(
    () => [
      ...
      { key: "revenue", label: "Revenue", render: (r) => revenueFormat(r?.revenue) },
      { key: "link", label: "Action", render: (r) => linkCell(r?.link) },
    ],
    []
  );
  return <DataTable title="Top Employees" columns={columns} data={data} emptyMessage="No employees found" />;
};
...
export const ClientsTable = ({ data }) => {
  ...
  return <DataTable title="Top Clients" columns={columns} data={data} emptyMessage="No clients found" />;
};
...
export const ExpiringSubscriptionsTable = ({ data }) => {
  ...
  return <DataTable title="Expiring Subscriptions" columns={columns} data={data} emptyMessage="No expiring subscriptions" />;
};
