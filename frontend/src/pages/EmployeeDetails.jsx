...
const EmployeeDetails = () => {
  ...
  const years = useMemo(() => {
    if (
      currentEmployee?.available_years &&
      currentEmployee.available_years.length > 0
    ) {
      return currentEmployee.available_years;
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div
        className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 transition-opacity duration-300 ${loading ? "opacity-50 pointer-events-none" : "opacity-100"}`}
      >
        <StatCard
          title="Revenu Mensuel"
          value={formatDH(stats.revenu_mensuel)}
          icon={DollarSign}
          subtext={`Période: ${months.find((m) => m.value === selectedMonth)?.label}`}
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 gap-5 transition-opacity duration-300 ${loading ? "opacity-50" : "opacity-100"}`}
      >
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-12 gap-8 items-start transition-opacity duration-300 ${loading ? "opacity-50" : "opacity-100"}`}
      >
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-muted-foreground flex items-center gap-2">
                    <Briefcase size={16} className="dark:text-foreground" />{" "}
                    Total Abonnements
                  </span>
                ...
                <div className="flex justify-between items-center text-sm">
                  <span className="text-muted-foreground flex items-center gap-2">
                    <Calendar size={16} className="dark:text-foreground" />{" "}
                    Abonnements Actifs
                  </span>
            ...
            {clientsRecents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <p className="text-muted-foreground text-xs mt-1">
                  Cet employé n'a pas encore de revenus pour cette période.
                </p>
              </div>
            ) : (
              <>
                        ...
                        >
                          <span className="flex items-center gap-1">
                            ...
                            <ArrowUpDown
                              size={14}
                              className={
                                sortConfig.key === "total_abonnements"
                                  ? "opacity-100"
                                  : "opacity-40"
                              }
                            />
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div
                                className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-xs ${getAvatarColor(client.nom)}`}
                              >
                  ...
                  <div className="px-6 py-4 border-t border-border flex items-center justify-between bg-muted/20">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1.5 rounded-md border border-border bg-background disabled:opacity-50"
                      >
                        <ChevronLeft size={16} />
                      </button>
                      <button
