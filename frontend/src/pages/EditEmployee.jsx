...
export default function EditEmployee() {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const updateToast = toast.loading(t("common.updating"));
    const result = await dispatch(updateEmployee({ id, data: formData }));
    if (updateEmployee.fulfilled.match(result)) {
      toast.success(t("employees.profileUpdated"), { id: updateToast });
    ...
    <div className="min-h-[calc(100vh-8rem)] flex items-center justify-center animate-in fade-in duration-500">
      ...
      <div className="w-full max-w-xl">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-foreground tracking-tight">{t("employees.editTitle")}</h1>
            <p className="text-muted-foreground mt-1 text-sm">{t("employees.editSubtitle")}</p>
          </div>
        ...
        <div className="bg-card text-card-foreground rounded-xl shadow-sm border border-border overflow-hidden">
          <div className="p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              {[
                { name: "nom", label: t("employees.name"), type: "text", icon: User },
                { name: "email", label: t("employees.email"), type: "email", icon: Mail },
              ].map((f) => (
                <div key={f.name}>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">
                  {t("auth.password")} <span className="text-muted-foreground font-normal text-xs ml-1">{t("employees.leaveBlank")}</span>
                </label>
                <div className="relative group">
              ...
              <div className="pt-2 flex justify-end">
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />} {t("employees.updateEmployee")}
                </button>
              </div>
            </form>
          </div>
