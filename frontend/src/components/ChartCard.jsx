...
export const RevenueTrendChart = ({ data }) => {
  ...
  const itemsPerPage = viewType === "mensuel" ? 12 : 5;
  ...
  const paginatedData = useMemo(() => {
    ...
    return dataset.slice(start, start + itemsPerPage).map((d) => ({
      label: d.label,
      revenue: d.total,
    }));
    ...
    <ChartCard
      ...
      action={
        <div className="flex flex-wrap items-center gap-3">
          ...
          {totalPages > 1 && (
            <div className="flex items-center gap-1.5 bg-muted/20 border border-border/50 rounded-md p-0.5">
              <button
                onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
                disabled={currentPage === 1}
                className="p-1 rounded text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-30 transition-colors"
              >
                <ChevronLeft size={16} />
              </button>
              <span className="text-xs font-medium text-muted-foreground min-w-[3.5rem] text-center select-none">
